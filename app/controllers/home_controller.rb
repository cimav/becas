    ...
    if session[:user_type].eql? 'User'
      if is_admin?
        @scholarships = Scholarship.where.not(status: Scholarship::DELETED).order("CASE
                                  ...
                                  WHEN status = #{Scholarship::TO_COMMITTEE} THEN 2
                                  WHEN status = #{Scholarship::APPROVED} THEN 3
                                  WHEN status = #{Scholarship::ACTIVE} THEN 4
                                  WHEN status = #{Scholarship::REJECTED} THEN 5
                                  WHEN status = #{Scholarship::INACTIVE} THEN 6
                                END")
